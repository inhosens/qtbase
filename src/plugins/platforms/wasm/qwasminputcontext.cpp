...
static void compositionUpdateCallback(emscripten::val event)
{
    ...
    // WA for IOS.
    // Not sure now because I cannout test it anymore.
//    int replaceSize = 0;
...
void QWasmInputContext::showInputPanel()
{
    ...
    const auto styleLeft = std::to_string(cursorRectangle.left() + window->x()) + "px";
    const auto styleTop = std::to_string(cursorRectangle.top() + window->y()) + "px";
